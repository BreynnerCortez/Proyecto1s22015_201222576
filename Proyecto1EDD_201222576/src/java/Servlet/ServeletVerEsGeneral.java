 ...
 * @author Breynner
 */
@WebServlet(name = "ServeletVerEsGeneral", urlPatterns = {"/ServeletVerEsGeneral"})
public class ServeletVerEsGeneral extends HttpServlet {
...
"         </style>");
    ...
    private boolean limpiaresgeneral() {
        // Note that the injected javax.xml.ws.Service reference as well as port objects are not thread safe.
        // If the calling of port operations may lead to race condition some synchronization is required.
        estructuras.WSEstacionGeneral port = service.getWSEstacionGeneralPort();
        return port.limpiaresgeneral();
    }
