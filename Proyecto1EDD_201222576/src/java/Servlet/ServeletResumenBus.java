 ...
 * @author Breynner
 */
@WebServlet(name = "ServeletResumenBus", urlPatterns = {"/ServeletResumenBus"})
public class ServeletResumenBus extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        imprimirchofer();
        limpiarchofer();
        String cuerpo="IDBus          Horarios\n";
        String idbusresu=request.getParameter("idresu");
        String []choferes=imprimirchofer().split(",");
        for(int i=0;i<choferes.length;i++){
            resumenbus(choferes[i], idbusresu);
...
"         </style>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h2><pre>"+cuerpo+"</pre></h2>");
    ...
    private String resumenbus(java.lang.String arg0, java.lang.String arg1) {
        // Note that the injected javax.xml.ws.Service reference as well as port objects are not thread safe.
        // If the calling of port operations may lead to race condition some synchronization is required.
        estructuras.WSChofer port = service.getWSChoferPort();
        return port.resumenbus(arg0, arg1);
    }

    private boolean limpiaresumenbus(java.lang.String arg0) {
        // Note that the injected javax.xml.ws.Service reference as well as port objects are not thread safe.
        // If the calling of port operations may lead to race condition some synchronization is required.
        estructuras.WSChofer port = service.getWSChoferPort();
        return port.limpiaresumenbus(arg0);
    }
